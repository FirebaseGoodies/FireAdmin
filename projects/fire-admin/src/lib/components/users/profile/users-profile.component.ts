...
import { PostData } from '../../../models/collections/post.model';
...
export class UsersProfileComponent implements OnInit, OnDestroy {
  ...
  user: User;
  allRoles: object = {};
  latestPosts: Observable<PostData[]>;
    ...
    this.latestPosts = this.posts.getWhere(this.postsLanguage + '.createdBy', '==', this.user.id, true).pipe(
      map((posts: PostData[]) => {
        ...
        if (this.postsLanguage) {
          posts = posts.filter((post: PostData) => post.lang === this.postsLanguage);
        }
        return posts.slice(0, 5).sort((a: PostData, b: PostData) => b.createdAt - a.createdAt);
      }),
